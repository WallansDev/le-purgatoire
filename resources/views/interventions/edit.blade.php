                    ...
                    <form action="{{ route('interventions.update', $intervention) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <select id="technician_id" name="technician_id"
                                    ...
                                    @foreach ($technicians as $technician)
                                        <option value="{{ $technician->id }}"
                                            {{ old('technician_id', $intervention->technician_id) == $technician->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <x-text-input id="scheduled_at" class="block mt-1 w-full" type="datetime-local"
                                    name="scheduled_at" :value="old('scheduled_at', $intervention->scheduled_at?->format('Y-m-d\TH:i'))" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="title" class="block mt-1 w-full" type="text" name="title"
                                    :value="old('title', $intervention->title)" required autofocus />
                                <x-input-error :messages="$errors->get('title')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="started_at" :value="__('Date et heure de début')" />
                                <x-text-input id="started_at" class="block mt-1 w-full" type="datetime-local"
                                    name="started_at" :value="old('started_at', $intervention->started_at?->format('Y-m-d\TH:i'))" />
                                <x-input-error :messages="$errors->get('started_at')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="finished_at" :value="__('Date et heure de fin')" />
                                <x-text-input id="finished_at" class="block mt-1 w-full" type="datetime-local"
                                    name="finished_at" :value="old('finished_at', $intervention->finished_at?->format('Y-m-d\TH:i'))" />
                            ...
                            <div>
                                ...
                                <div class="mt-2">
                                    <label class="inline-flex items-center mb-2">
                                        <input type="checkbox" id="no_note" name="no_note" value="1"
                                            {{ old('no_note', is_null($intervention->note)) ? 'checked' : '' }}
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                        <span class="ml-2 text-sm text-gray-600">Non noté</span>
                                    </label>
                                </div>
                                <x-text-input id="note" class="block mt-1 w-full" type="number" name="note"
                                    :value="old('note', $intervention->note !== null ? $intervention->note : '')" min="0" max="5" />
                            ...
                            <div>
                                ...
                                <div class="mt-2">
                                    <label class="inline-flex items-center mb-2">
                                        <input type="checkbox" id="no_service_note" name="no_service_note"
                                            value="1"
                                            {{ old('no_service_note', is_null($intervention->service_note)) ? 'checked' : '' }}
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                        <span class="ml-2 text-sm text-gray-600">Non notée</span>
                                    </label>
                                </div>
                                <x-text-input id="service_note" class="block mt-1 w-full" type="number"
                                    name="service_note" :value="old(
                                        'service_note',
                                        $intervention->service_note !== null ? $intervention->service_note : '',
                                    )" min="0" max="5" />
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="4"
                                    class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $intervention->description) }}</textarea>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="address" name="address" rows="3"
                                    class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                    placeholder="Ex: 123 Rue de la République, 75001 Paris">{{ old('address', $intervention->address) }}</textarea>
                                ...
                                <select id="tags" name="tags[]" multiple
                                    ...
                                    @php
                                        $selectedTags = collect(
                                            old('tags', $intervention->tags->pluck('id')->toArray()),
                                        );
                                    @endphp
                                    @foreach ($tags as $tag)
                                        <option value="{{ $tag->id }}"
                                            data-color="{{ $tag->color ?? '#4f46e5' }}"
                                ...
                                <div class="mt-2">
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" id="is_completed" name="is_completed" value="1"
                                            {{ old('is_completed', $intervention->is_completed) ? 'checked' : '' }}
                                            class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                        <span class="ml-2 text-sm text-gray-600">Oui, l'intervention est
                                            terminée</span>
                                    </label>
                                </div>
                            ...
                            <div class="md:col-span-2" id="non_completion_reason_field"
                                style="display: {{ old('is_completed', $intervention->is_completed) ? 'none' : 'block' }};">
                                <x-input-label for="non_completion_reason" :value="__('Raison de non-complétion *')" />
                                <textarea id="non_completion_reason" name="non_completion_reason" rows="3"
                                    class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                    placeholder="Expliquez pourquoi l'intervention n'a pas été terminée...">{{ old('non_completion_reason', $intervention->non_completion_reason) }}</textarea>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="notes" name="notes" rows="4"
                                    class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                    placeholder="Notes internes sur l'intervention...">{{ old('notes', $intervention->notes) }}</textarea>
                                <x-input-error :messages="$errors->get('notes')" class="mt-2" />
                            </div>

                            <div class="md:col-span-2">
                                <x-input-label for="client_comments" :value="__('Commentaires du client')" />
                                <textarea id="client_comments" name="client_comments" rows="4"
                                    class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                                    placeholder="Commentaires du client sur l'intervention...">{{ old('client_comments', $intervention->client_comments) }}</textarea>
